...
async function addNewsTable() {
  try {
    ...
    if (!tableExists.rows[0].exists) {
      ...
      console.log('Inserting existing news data...');
      ...
      for (const [id, title, message, posted_by, date_posted] of newsData) {
        await query(`
          INSERT INTO news (id, title, message, posted_by, date_posted)
          VALUES ($1, $2, $3, $4, $5)
        `, [id, title, message, posted_by, date_posted]);
      }
      
      console.log(`✅ Inserted ${newsData.length} news posts successfully!`);
    ...
    // Get record count
    const result = await query(`SELECT COUNT(*) as count FROM news`);
    console.log(`📊 News table contains ${result.rows[0].count} records`);
    
    console.log('🎉 News system migration completed successfully!');
